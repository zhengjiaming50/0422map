  ...
  <div v-if="restaurant" class="restaurant-info">
    ...
    <div class="reviews-section">
      ...
      <div class="average-rating">
        ...
        <span class="reviews-count">{{ reviewsCount }}条评价</span>
      </div>
      ...
      <div class="add-review-form">
        ...
        <textarea placeholder="说说你的体验（可选）"></textarea>
...
| reviews | Computed | 餐厅评价列表 |
| averageRating | Computed | 餐厅平均评分 |
| close | Event | 关闭详情面板事件 |
| submitReview | Method | 提交新评价方法 |

**样式说明**
| 类名 | 描述 |
|------|------|
| .restaurant-info | 餐厅详情容器，白色背景，圆角边框，最大高度80vh可滚动 |
| .restaurant-header | 标题区域，深蓝色背景，显示餐厅名称和美食类型 |
| .restaurant-image | 餐厅图片容器，固定高度180px |
| .restaurant-details | 详情内容区域，包含餐厅基本信息 |
| .reviews-section | 用户评价部分，包含平均评分、星级评分、评价列表和评价表单 |
| .average-rating | 平均评分容器，显示平均评分和评价数量 |
| .rating-value | 评分值，显示为数字 |
| .rating-stars | 星级评分显示，使用星星图标表示评分 |
| .reviews-count | 评价数量，显示为文本 |
| .add-review-form | 评价表单容器，包含星级评分选择器、评价输入框和提交按钮 |
| .close-btn | 关闭按钮样式，红色背景白色文字 |
...
[
  {
    ...
    "restaurant_id": 1,
    "rating": 5,
    "comment": "这家餐厅的热干面非常地道，价格也实惠！",
    "user_name": "美食爱好者",
    "created_at": "2023-04-25T13:45:22"
  },
  {
    // 其他评价...
  }
...
{
  "rating": 4,
  "comment": "非常好吃，服务也很周到，但是环境可以再改善一下",
  "user_name": "小明"
}
```
...
```json
{
  ...
  "comment": "非常好吃，服务也很周到，但是环境可以再改善一下",
  "user_name": "小明",
  "created_at": "2023-04-25T14:32:45"
}
```
...
|------|------|------|------|
| id | Integer | 是 | 主键 |
| restaurant_id | Integer | 是 | 外键，关联餐厅ID |
| rating | Integer | 是 | 评分(1-5) |
| comment | Text | 否 | 评价内容 |
| user_name | String(50) | 否 | 用户名称 |
| created_at | DateTime | 自动 | 创建时间 |
...
| getRestaurantReviews | restaurantId: Number | Promise<Array> | 获取指定餐厅的评价 |
...
| restaurantReviews | Object | 餐厅评价数据，键为餐厅ID |
| averageRatings | Object | 餐厅平均评分，键为餐厅ID |
...
| fetchRestaurantReviews | restaurantId: Number | 获取指定ID的餐厅评价 |
| submitRestaurantReview | restaurantId: Number, reviewData: Object | 提交餐厅评价 |
