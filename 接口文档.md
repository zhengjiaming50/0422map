...
**结构与功能**
- 页面分为三个主要内容区块：武汉美食文化简介、特色名菜展示、美食街区介绍
- 顶部导航可快速跳转至各内容区块
- 特色名菜展示包括热干面、豆皮、武汉鸭脖等，每种美食提供图片、介绍和文化背景
- 美食街区介绍包括户部巷、司门口美食街、吉庆街等著名美食街区
- 页面底部提供链接引导用户查看一日美食路线推荐

### FoodRouteView
```vue
美食路线推荐页面，提供多条精选一日美食路线
<template>
...
| 名称 | 类型 | 描述 |
|------|------|------|
| activeRoute | ref | 当前选中的路线索引 |
| routes | ref | 多条美食路线数据，包括经典早点之旅、江滨美食探索之旅、网红美食打卡之旅 |

**路线数据结构**
- 每条路线包含：名称、持续时间、美食数量、地点数量、路线描述、停留点列表、贴士列表
- 每个停留点包含：时间、名称、类型、地址、图片、描述、推荐菜品、小贴士
...
| `/stats` | stats | StatsView | 统计页面 |
| `/food-culture` | food-culture | FoodCultureView | 美食文化专题页面 |
| `/food-route` | food-route | FoodRouteView | 美食路线推荐页面 |
