...
| 名称 | 类型 | 描述 |
|------|------|------|
| isActive | Boolean (prop) | 是否显示面板 |
| userLocation | Object (prop) | 用户当前位置 |
| close | Event | 关闭面板事件 |
| start-picking-mode | Event | 开始位置选择模式事件 |
| route-planned | Event | 路线规划完成事件 |
| route-cleared | Event | 路线清除事件 |
| setPickedLocation | Method | 设置选中的位置 |
| clearRoute | Method | 清除当前路线 |
...
```
GET /api/restaurants/heatmap-data
```
...
| getRoute | routeParams: Object | Promise<Object> | 获取路线规划 |
