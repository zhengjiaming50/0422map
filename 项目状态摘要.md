...
└── src/
    ...
    │   ├── RestaurantInfo.vue # 餐厅详情组件（已更新添加评价功能）
    │   └── RestaurantMarker.vue # 餐厅标记组件
    ├── router/             # 路由配置
    ├── services/           # API服务
    │   └── api.js          # 餐厅API服务（已更新添加评价API）
    ├── stores/             # 状态管理
    │   └── restaurant.js   # 餐厅数据状态管理（已更新添加评价状态）
    ├── views/              # 页面组件
    │   ├── HomeView.vue    # 首页组件
    │   └── MapView.vue     # 地图页面组件
    ├── App.vue             # 主应用组件
...
| 接口路径 | 方法 | 描述 |
|----------|------|------|
| /api/restaurants/:id/reviews | GET | 获取指定餐厅的评价列表 |
| /api/restaurants/:id/reviews | POST | 为指定餐厅添加新评价 |
